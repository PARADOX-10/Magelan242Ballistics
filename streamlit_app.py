...
st.set_page_config(page_title="Magelan Apex v110", layout="centered")
...
class ApexEngine:
    ...
    def calculate(self):
        ...
        # Вибір драг-моделі (G1 зазвичай потребує корекції коефіцієнта)
...
# --- ІНТЕРФЕЙС ---
st.markdown("<h1 style='text-align: center;'>APEX PREDATOR v110</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    drag_model = st.radio("Драг-модель кулі", ["G7", "G1"], help="G7 для човноподібних куль (VLD), G1 для класичних плоскодонних.")
    v0_in = st.number_input("V0 швидкість (м/с)", value=893.0)
    bc_in = st.number_input(f"БК кулі ({drag_model})", value=0.292, format="%.3f")
    w_in = st.number_input("Вага кулі (гран)", value=195.0)
    twist_in = st.number_input("Твіст (1:n)", value=11.0)
    sh_in = st.number_input("Висота прицілу (см)", value=5.0)
    click_val = st.selectbox("Ціна кліка (MIL)", [0.1, 0.05], index=0)
    p_sens_in = st.slider("Термозалежність (% на 10°C)", 0.0, 3.0, 0.7)
    
    st.header("🗺️ ГЕОПОЗИЦІЯ")
    lat_in = st.number_input("Широта", value=50.0)
    az_in = st.slider("Азимут (градуси)", 0, 360, 90)
...
params = {
    ...
    'p_sens': p_sens_in, 'lat': lat_in, 'az': az_in, 'drag_model': drag_model,
...
with c1:
    st.markdown(f"""<div class="main-card">
        ...
        <div class="sub-value">{int(round(res['v_mil']/click_val))} КЛІКІВ</div>
...
with c2:
    st.markdown(f"""<div class="main-card">
        ...
        <div class="sub-value">{int(round(res['h_mil']/click_val))} КЛІКІВ</div>
    </div>""", unsafe_allow_html=True)
...
st.caption(f"Коригована швидкість V0: {res['v0_actual']} м/с | Час польоту: {res['tof']} с")
